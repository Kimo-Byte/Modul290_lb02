...
module.exports = class Customer {
    ...
    updateById(id,customer, cbResult){
        let queryString = 'Update customer SET email = ?, firstName = ?, lastName = ?, password = ?, phone = ?, message = ?,';
        queryString += ' WHERE id = ?';
        sql.query(queryString,
            [customer.email, customer.firstName, customer.lastName, customer.password, customer.phone, customer.message, parseInt(id)],
            (err, result) => {
                if (err){
                    console.log("error: ", err);
                    //Return error, data = null
